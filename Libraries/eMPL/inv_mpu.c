...
#define min(a,b) ((a<b)?a:b)
#define i2c_write   MPU9250_SPIx_Writes
#define i2c_read    MPU9250_SPIx_Reads
static __inline int reg_int_cb(struct int_param_s *int_param)
{
	return 0;
}
...
static int compass_self_test(void)
{
	...
	if (MPU9250_AK8963_SPIx_Writes(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp))
		return 0x07;
	tmp[0] = AKM_BIT_SELF_TEST;
	if (MPU9250_AK8963_SPIx_Writes(st.chip_cfg.compass_addr, AKM_REG_ASTC, 1, tmp))
		goto AKM_restore;
	...
	do {
		delay_ms(10);
		if (MPU9250_AK8963_SPIx_Reads(st.chip_cfg.compass_addr, AKM_REG_ST1, 1, tmp))
			goto AKM_restore;
	...
	if (!(tmp[0] & AKM_DATA_READY))
		goto AKM_restore;
	if(MPU9250_AK8963_SPIx_Reads(st.chip_cfg.compass_addr, AKM_REG_HXL, 6, tmp))
		goto AKM_restore;
...
AKM_restore:
	...
	MPU9250_AK8963_SPIx_Writes(st.chip_cfg.compass_addr, AKM_REG_ASTC, 1, tmp);
	tmp[0] = SUPPORTS_AK89xx_HIGH_SENS;
	MPU9250_AK8963_SPIx_Writes(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp);
	mpu_set_bypass(0);
	return result;
}
#endif
...
static int setup_compass(void)
{
	...
	for (akm_addr = 0x0C; akm_addr <= 0x0F; akm_addr++) {
		...
		result = MPU9250_AK8963_SPIx_Reads(akm_addr, AKM_REG_WHOAMI, 1, data);
	...
	if (MPU9250_AK8963_SPIx_Writes(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, data))
		return -1;
	delay_ms(1);
	...
	if (MPU9250_AK8963_SPIx_Reads(st.chip_cfg.compass_addr, AKM_REG_ASAX, 3, data))
		return -1;
...
#ifdef AK89xx_BYPASS
	if (MPU9250_AK8963_SPIx_Reads(st.chip_cfg.compass_addr, AKM_REG_ST1, 8, tmp))
		return -1;
	tmp[8] = AKM_SINGLE_MEASUREMENT;
	if (MPU9250_AK8963_SPIx_Writes(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp+8))
		return -1;
#else
